...
class contract extends dbStatusObj
{
	...
	protected $obj_values = ['id', 'basic', 'basic_currency_id','basic_currency', 'basic_currency_code', 'basic_currency_symbol', 'basic_recurrence',
							'basic_recurrence_id', 'base_hours', 'base_hours_recurrence', 'base_hours_recurrence_id', 'ot_eligible', 'position_id', 'position',
							'soc', 'eoc'];
	...
	public function statusAtDate($date = false)
	{
		...
		if(!isset($this->stmt_status_at_date))
		{
			...
			SELECT a.`id`, a.`basic`, a.`basic_currency`, a.`basic_recurrence`, a.`base_hours`,
					a.`base_hours_recurrence`, a.`ot_eligible`, a.`position`, a.`soc`, a.`eoc`,
					b.`name`, b.`code`, b.`symbol`, c.`period`, d.`period`, e.`name`
			FROM `contracts` a
				LEFT JOIN
					`currencies` b
				ON
			...
			$this->stmt_status_at_date->bind_result($this->id, $this->basic, $this->basic_currency_id, $this->basic_recurrence_id, $this->base_hours, $this->base_hours_recurrence_id,
				$this->ot_eligible, $this->position_id, $this->soc, $this->eoc, $this->basic_currency, $this->basic_currency_code, $this->basic_currency_symbol, $this->basic_recurrence,
				$this->base_hours_recurrence, $this->position);
		}
